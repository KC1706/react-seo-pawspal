...
<div id="navbar"></div>
  ...
  <header className="hero">
    <div className="wrap g2">
      <div className="hero-content">
        <div className="hero-tag" style={{'background': 'rgba(255, 255, 255, .12)', 'padding': '6px 16px', 'borderRadius': '50px', 'fontSize': '.82rem', 'fontWeight': '600', 'color': 'var(--white)', 'borderColor': '#fca5a5', 'marginBottom': '14px', 'display': 'inline-block'}}>EMERGENCY & MOBILITY</div>
        <h1>Instant Pet Travel Clearance & 24/7 Emergency Vet Locator</h1>
        <p className="lead">Whether you're flying internationally with your dog or facing a midnight medical crisis, the PAWID infrastructure ensures zero delays when time and compliance matter most.</p>
        <div style={{'display': 'flex', 'gap': '14px', 'marginTop': '30px'}} className="btn-wrap" >
          <a href="/pet-identity/pet-digital-identity#create-pawid" className="btn btn-amber btn-text">Protect Your Pet Today →</a>
          <a href="#features" className="btn btn-ghost-w btn-text">View Emergency Tools</a>
        </div>
      </div>
      <div style={{'borderRadius': '20px', 'overflow': 'hidden', 'border': '1px solid rgba(26,58,42,.08)', 'boxShadow': '0 8px 40px rgba(26, 58, 42, .12)', 'display': 'flex', 'aspectRatio': '4/3'}}><img src="/assets/seo_pet_travel.png" alt="PawsPal Module" style={{'width': '100%', 'height': '100%', 'objectFit': 'cover'}} /></div>
    </div>
  </header>
  ...
  <section className="section" id="features">
    <div className="wrap">
      <div style={{'textAlign': 'center', 'marginBottom': '56px'}}>
        <span className="feature-label">The Safety Net</span>
        <h2>Always Ready for the Unexpected</h2>
        <p style={{'margin': '0 auto'}}>Connected directly to the PawsPal GPS network, your app is highly aware of your surroundings and your pet's precise health vulnerabilities.</p>
      </div>

      <div className="g3">
        <div className="card">
          <div className="card-icon">✈️</div>
          <h3>Export Certificate Checker</h3>
          <p>Planning to travel? Our automated system checks your pet's PAWID health record against the import regulations of 40+ countries to clear them for boarding.</p>
        </div>
        <div className="card">
          <div className="card-icon">🏥</div>
          <h3>24/7 Verified Vet Radar</h3>
          <p>Panic in the middle of the night? Tap the SOS button. We geo-locate the nearest verified, open 24/7 veterinary clinic from our network of 2,400+ providers.</p>
        </div>
        <div className="card">
          <div className="card-icon">🪪</div>
          <h3>Rabies Compliance Tracking</h3>
          <p>Strict tracking of Rabies Titer limits and vaccination bounds, automatically generating the documentation you need for Indian Railway pet travel.</p>
        </div>
      </div>
    </div>
  </section>
  ...
  <section className="section" style={{'background': 'var(--cream)', 'textAlign': 'center'}}>
    <div className="wrap">
      ...
      <p style={{'margin': '0 auto 30px'}}>Create your pet's immutable PAWID identity profile so their medical data is always ready when you critically need it.</p>
      <a href="/pet-identity/pet-digital-identity#create-pawid" className="btn btn-amber">Create PAWID Profile →</a>
    </div>
  </section>
